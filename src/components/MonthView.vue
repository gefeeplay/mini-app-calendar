    ...
    <div class="month-view">
        ...
        <div class="days">
            <div v-for="day in getMonthDays(curMonth, year)" :key="day.key" class="day" :class="{ empty: !day.number }">
                ...
                <!-- отображаем событие -->
                <div v-if="day.number && getEvents(day.number).length" class="event"
                    v-for="(ev, i) in getEvents(day.number)" :key="i">
                    {{ ev }}
                </div>
            </div>
        </div>

        <DayView ref="eventDialog" :day="currentDay" :monthName="monthNames[curMonth]" :events="getEvents(currentDay)"
            @close="closeDialog" @save="saveEvent" @delete="deleteEvent" />
